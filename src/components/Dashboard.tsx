...
const Dashboard = () => {
  return (
    <section id="dashboard-demo" className="py-12 md:py-20 px-4 bg-gradient-to-br from-gray-50 to-white relative overflow-hidden">
      ...
      <div className="absolute top-20 left-10 w-12 h-12 md:w-20 md:h-20 bg-brand-green/5 rounded-full"></div>
      <div className="absolute bottom-20 right-10 w-10 h-10 md:w-16 md:h-16 bg-blue-500/5 rounded-full"></div>
      <div className="absolute top-1/2 left-1/4 w-8 h-8 md:w-12 md:h-12 bg-purple-500/5 rounded-full animate-bounce delay-1000"></div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-4 md:p-8 max-w-4xl mx-auto hover:shadow-3xl transition-all duration-500 animate-in scale-in-95 duration-700 delay-300 group">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-6 md:mb-8">
            <div className="bg-brand-green/10 rounded-lg p-4 md:p-6 text-center hover:bg-brand-green/15 transition-all duration-300 hover:scale-105 hover:shadow-lg animate-in slide-in-from-left-6 duration-500 delay-500 group">
              <div className="text-2xl md:text-3xl font-bold text-brand-green mb-2 group-hover:scale-110 transition-transform duration-300">1.247</div>
          ...
          <div className="bg-gray-50 rounded-lg p-4 md:p-6 hover:bg-gray-100 transition-colors duration-300 animate-in fade-in-50 duration-700 delay-1100">
            <h3 className="font-bold text-brand-dark mb-4 animate-in slide-in-from-left-4 duration-500 delay-1200 text-lg md:text-xl">Conversas recentes</h3>
              ...
              {[
                ...
                >
                  <div className="flex items-center space-x-3">
                    <div className="w-8 h-8 md:w-10 md:h-10 bg-brand-green rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                      <span className="text-white font-bold text-xs md:text-sm">
                        {conversation.name.charAt(0)}
                      </span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-brand-dark text-sm md:text-base group-hover:text-brand-green transition-colors duration-300 truncate">{conversation.name}</p>
                      <p className="text-brand-gray text-xs md:text-sm truncate">{conversation.message}</p>
                    </div>
                  </div>
                  <span className="text-xs text-brand-gray group-hover:text-brand-dark transition-colors duration-300 flex-shrink-0">{conversation.time}</span>
                </div>
              ))}
            </div>
          </div>
